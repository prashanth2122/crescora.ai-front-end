...
import { ArrowRight, BellRing, CalendarDays, CreditCard, LifeBuoy, MessageSquareText, RefreshCcw, Route, Star } from "lucide-react";
import { createExactPageMetadata } from "@/lib/seo";

export const metadata = createExactPageMetadata({
  title: siteContent.industries.serviceBusinesses.metadata.title,
  description: siteContent.industries.serviceBusinesses.metadata.description,
  path: "/industries/service-businesses",
});

export default function ServiceBusinessesIndustryPage() {
  const page = siteContent.industries.serviceBusinesses;
  const workflowIcons = [CalendarDays, BellRing, CreditCard, LifeBuoy, Route, MessageSquareText, Star, RefreshCcw];
    ...
    <PageShell>
      <PageHero
        ...
        visual={
          <div className="overflow-hidden rounded-[2rem] border border-zinc-200 bg-[linear-gradient(180deg,rgba(255,255,255,0.94)_0%,rgba(244,244,245,0.96)_100%)] p-6 shadow-[0_24px_80px_rgba(15,23,42,0.08)] sm:p-7">
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-zinc-500">Service workflow path</p>
      ...
      <section className="mx-auto w-full max-w-7xl px-4 pb-16 sm:px-6 lg:px-8">
        <div className="grid gap-4 lg:grid-cols-2">
          {page.overviewCards.map((card, index) => {
            const Icon = index === 0 ? CalendarDays : CreditCard;
      ...
      <section className="mx-auto w-full max-w-7xl px-4 pb-20 sm:px-6 lg:px-8">
        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <Card className="border-zinc-200 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.05)]">
            <CardContent className="p-6 sm:p-8">
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-zinc-500">Operational coverage</p>
