...
- contact details are shown on the contact page plus the footer for sales, support, direct contact, and office location; the direct contact phone is read from `CONTACT_PHONE_NUMBER` in `src/lib/app-config.ts` and falls back to the configured support or WhatsApp number
